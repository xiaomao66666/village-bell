...
截图使用人工演示数据，村庄插画不代表真实阵型。

| 村庄总览 | 七日图表 | 完成时间轴 |
| --- | --- | --- |
| ![村庄总览](docs/screenshots/dashboard.png) | ![七日图表](docs/screenshots/forecast.png) | ![完成时间轴](docs/screenshots/timeline.png) |

新增游戏原图、任务详情、名称/ID 搜索、只看今天、隐藏已完成、同步前后数量对比、提前提醒、免打扰与桌面小组件。建筑按导出等级匹配，未知等级使用通用图标。

| 游戏任务卡片 | 贴心提醒 | 桌面看板 |
| --- | --- | --- |
| ![游戏任务](docs/screenshots/game-tasks.png) | ![提醒设置](docs/screenshots/reminder-options.png) | ![桌面看板](docs/screenshots/widget.png) |

[素材来源、许可与匹配规则](docs/GAME-ASSETS.md)。游戏素材 © Supercell，本内容非官方且未经 Supercell 认可，参见 [粉丝内容政策](https://supercell.com/en/fan-content-policy/)。

## 安装与使用

1. 从 Releases 下载 APK（或自行构建 `dist/village-bell-0.3.0.apk`），传到手机打开安装。首次安装时，系统可能要求允许该文件来源安装应用。
...
当前管理一个村庄。重新导入会替换任务、取消旧计时、重置单项提醒开关。较旧的同村庄数据不能覆盖新数据；重复导入同一导出时间的数据不会重建提醒。历史快照中已经到期的条目只显示「预计已完成」，不会一次性弹出历史提醒。

## 已实现

- 原生中文安卓界面，村庄/提醒/指南三个页面，原创村庄插画与分类图标。
...
- 在「提醒」页选择提前 5/15/30/60 分钟，预计完成时仍单独提醒；提前提醒默认关闭。
- 开启免打扰后，时段内的提醒在结束时合并送达。开始与结束相同表示不限制，测试提醒仍会发送。免打扰默认关闭。
- 点击「添加桌面小组件」，或在桌面长按添加「村庄升级看板」。点击组件右上角「同步」可直接导入。
- 小组件在同步、升级通知、系统恢复和系统定期刷新时更新；定期刷新由系统安排，可能延迟。它显示的是上次导出推算的时间。
...
- 12 项可视化计算检查：同步后进度、完成边界、按本地日期统计七日任务与时区差异。
- 18 项提醒与筛选检查：跨午夜免打扰、边界、过期提醒、提前提醒与免打扰冲突、名称/ID 搜索、日期筛选。
- 2026-09-17：Android 10 模拟器实测游戏图片、详情、提前与完成通知、小组件显示和同步入口；免打扰下系统闹钟已延后至设定结束时刻。
- APK manifest 检查：无网络权限，启动 Activity 与三个提醒相关权限符合预期。
- Android 10 模拟器已完成安装、页面切换、截图检查与返回桌面后的 10 秒测试通知，检查过 320dp 小屏幕及 1.3 倍字体。**锁屏通知、重启恢复和厂商省电策略仍需真机验证**。
